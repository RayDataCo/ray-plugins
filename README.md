...
plugins/ab-domain-research/           # FILL brigade: stocks a cellar with domain competency + gold exemplars
  ...
  skills/cert-body-sourcing/ + standards-regulatory-sourcing/ + academic-ocw-sourcing/ + public-filings-exemplars/ + award-case-study-exemplars/
  evals/                              # two-arm execution-eval evidence (both rounds)
plugins/ab-marketing/                 # discipline brigade: first exemplar-graded generative station
  MENU.md + skills/mise|expo|service/ + skills/effectiveness-narrative-draft/ + evals/
```
...
- **`ab-domain-research`** — the **fill brigade** (kitchen kind, domain-centric analog of company research): stocks a knowledge cellar with domain **competency** and **gold exemplars** — the two things a generative-skill build needs, since generative skills have no computable oracle and get graded against exemplars. Standard surface (`mise` incl. a `CELLAR_ROOT` landing-target check, composing `expo`, `service`) over 5 eval-proven sourcing stations: `cert-body-sourcing` (+0.60), `standards-regulatory-sourcing` (+1.00), `academic-ocw-sourcing` (+0.60, CC-licensed-only gate), `public-filings-exemplars` (+0.40–0.60 — EDGAR exhibits/MD&A, SAM.gov RFP/SOWs, court briefs, patents, GAO/CRS, each exemplar with a `why_gold`), `award-case-study-exemplars` (+0.60 — creative-domain gold exemplars from juried award libraries), and `freshness-watch` (99% per-element — re-verifies landed grounding against PRIMARY sources on a cadence and routes factory refire tickets: EVAL-SPEC's world-change regression trigger, live). Every station enforces license-gate-before-content, authority tiering, per-source dispositions with reason classes, provenance frontmatter, and untrusted-data discipline (embedded instructions ⇒ `injection-suspect`). Curation, never bulk scraping; eval evidence in `evals/`.
- **`ab-marketing`** — marketing discipline brigade: the `mise` + `expo` + `service` surface over eval-proven marketing stations. **Three live generative stations — the marketplace's first exemplar-graded generative skills** (eval rubrics derived from real GAO campaign-evaluation + S-1 positioning gold sourced by ab-domain-research the same morning): `effectiveness-narrative-draft` (+1.00), `positioning-narrative-draft` (+0.80/+1.00 across rounds; round-1's critic-caught license violation fixed and re-verified), `marketing-brief-draft` (+0.80 both rounds). Plus `advertising-claims-endorsement-review` (structural FTC 255 coverage review, base 33%→93% per-element under the founder-ruled measure-skill-quality bar). `MENU.md` holds an honest 4-round evidence trail for the 2 still-held structural stations. Structural stations report elements, never legal advice. Only eval-passers ship.
