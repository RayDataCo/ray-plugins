...
- **`ab-marketing`** — marketing discipline brigade: the `mise` + `expo` + `service` surface over eval-proven marketing stations. First live: `effectiveness-narrative-draft` (+1.00 — campaign-effectiveness narratives/measurement plans in the findings-evidence-recommendation discipline, **the marketplace's first exemplar-graded generative station**: eval rubrics derived from real GAO campaign-evaluation gold sourced by ab-domain-research the same morning). `MENU.md` holds honest held-for-refire rows with real measured evidence for 5 more (incl. positioning-narrative at +1.00 held on a critic-caught license violation). Structural FTC coverage stations report elements, never legal advice. Only eval-passers ship.
