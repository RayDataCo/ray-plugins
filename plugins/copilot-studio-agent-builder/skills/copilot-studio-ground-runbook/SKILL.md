---
name: copilot-studio-ground-runbook
description: Re-ground the Copilot Studio generic runbook in the latest Microsoft Learn docs and report a sourced diff (the field changes fast — refresh instead of trusting a frozen asset). Use when the user wants to refresh, update, ground, re-verify, or freshen the Copilot Studio runbook against current Microsoft documentation.
---

# Re-ground the Copilot Studio runbook in the latest Microsoft docs

Re-ground the Copilot Studio **generic runbook** against the LATEST Microsoft Learn documentation. The product changes fast (preview features, Copilot Credits pricing, model availability, UI), so refresh before relying on it. (Part of the `copilot-studio-agent-builder` plugin.)

**Source files** (bundled in this plugin under `${CLAUDE_PLUGIN_ROOT}/skills/copilot-studio-agent-builder/`):
...
If `${CLAUDE_PLUGIN_ROOT}` doesn't resolve, locate the `copilot-studio-agent-builder` plugin's `skills/copilot-studio-agent-builder/reference/` directory.
...
4. Apply the verified updates to `generic-runbook.md` (and `copilot-studio-givens.md` where a fact changed) and bump the "Last grounded against Microsoft Learn" date at the bottom of the runbook to today.
   - **Claude Code:** edit the files in place. If a pulled project copy also exists (from the copilot-studio-pull-runbook skill), ask which to update, or update both.
