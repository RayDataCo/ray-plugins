...
- **Execution-eval station** — *wired + first measurement run* ([`skills/execution-eval-station/`](./skills/execution-eval-station/), [`workflow/execution-eval-variance-analysis.run.js`](./workflow/execution-eval-variance-analysis.run.js), result in [`examples/variance-analysis/execution-eval-report.md`](./examples/variance-analysis/execution-eval-report.md)). First lift on `variance-analysis` vs base Opus 4.8 = **0 pp** — both arms 100% on all four oracles, i.e. a *non-discriminating* fixture set (base model already at the ceiling), not a skill verdict. Remaining: add discriminating fixtures and/or a weaker-model baseline arm to actually surface lift; persist regression baselines to the rail; have the orchestrator consume the lift signal as an advance/kill/refire decision.
