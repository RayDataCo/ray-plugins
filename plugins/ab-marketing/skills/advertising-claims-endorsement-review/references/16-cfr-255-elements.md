...
2024 amendments (final rule announced 2023-06-29, codified 2024). Retrieved
2026-07-10. License: public domain, 17 U.S.C. § 105 — US federal government work,
full text excerptable with citation, no restriction to record.

Each element below carries a plain-language `requirement_summary` (safe to restate
freely), a `quotable_line` (safe to quote directly in a report's `citation` field,
tagged "public domain, 17 U.S.C. § 105"), and — where the element is easy to
mis-score — a `status_decision_rule` spelling out exactly when it is PRESENT vs.
ABSENT vs. DEFICIENT vs. NOT APPLICABLE (not-triggered). Do not invent additional
16 CFR 255 language beyond what's here — if a report needs a citation this file
doesn't cover, fall back to the general section reference (e.g. "16 CFR 255")
without a quote rather than fabricating one.
...
- quotable_line: "Defines 'endorsement' (any advertising message consumers are likely
...
- quotable_line: "An endorsement must reflect the honest opinions, findings, beliefs,
...
- quotable_line: "Advertisers are subject to liability for false or unsubstantiated
...
- quotable_line: "When an advertisement represents that an endorser's experience is
...
- quotable_line: "An expert endorser's qualifications must in fact give the expertise
...
- status_decision_rule: For a finished piece, mark DEFICIENT (not ABSENT) when the
  ...
  the trigger firing.
- quotable_line: "An organization's endorsement must reflect the collective judgment
...
- status_decision_rule: Mark ABSENT when a material connection is confirmed to exist
  ...
  any other element.
- quotable_line: "Any material connection between an endorser and the advertiser that
...
## 255.6 — Liability of advertisers, endorsers, and intermediaries (always evaluated)

- requirement_summary: Liability for a deceptive endorsement is not limited to the
  advertiser. Endorsers themselves, and intermediaries such as ad agencies, PR firms,
  review brokers, and reputation-management companies, can each be held liable for
  their own role in creating or spreading an endorsement they knew or should have
  known was deceptive — including orchestrating or failing to catch undisclosed paid
  or incentivized reviews.
- status_decision_rule: This element allocates liability across parties; it is not a
  content checklist the submitted material must affirmatively satisfy. Default to
  PRESENT unless the content itself shows a concrete intermediary (agency, PR firm,
  review broker, reputation-management company) orchestrating, instructing, or
  ignoring a deceptive or undisclosed-connection endorsement — only then mark
  DEFICIENT/ABSENT, quoting the specific evidence. Do not mark this DEFICIENT merely
  because the submitted content is silent on advertiser training or monitoring
  practices (silence is the default state, not a defect), and do not mark it
  DEFICIENT merely because a separate 255.5 finding is ABSENT or DEFICIENT — an
  undisclosed connection scored under 255.5 is not, by itself, intermediary-conduct
  evidence for this element.
- quotable_line: "Advertisers, endorsers, and intermediaries (ad agencies, PR firms,
  review brokers, reputation-management companies) can each be held liable for their
  own role in creating or disseminating an endorsement they know or should know is
  deceptive, including undisclosed paid/incentivized reviews"
...
255.0, 255.1-honest-opinion, 255.1-substantiation, and 255.6 are never conditional —
evaluate them on every review regardless of what else triggers.
