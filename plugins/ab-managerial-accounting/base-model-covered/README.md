...
draft skill built by the skill-agent-brigade) against that task's oracle fixtures — n=3 samples
...
- **Any fixture regression → promote the task to a built skill.** If the new base model drops
  ...
  registry. Route it through the skill-agent-brigade factory (spec → tests → author ⇄ critic →
